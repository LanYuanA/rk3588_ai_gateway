...
namespace
{
    ...
    public:
         ...
         */
        bool ensure(size_t requested_size) // 确保内存大于requested_size
        {
            if (requested_size == 0)
            {
                return false;
    ...
    RgaYuvRuntimeCache &cacheForYuvContext(RgaYuvConverterContext *context) {
      //每个线程都有一个哈希表，其中key是YUV转换器的指针，value是转换器对应的RGA缓存，放的是输入和输出缓存
...
//为这个stream_id线程初始化rga配置上下文，并且给他配置好具体用哪个rga内核
                                      ...
                                      RgaYuvConverterContext &context)
{
    ...
    if (!context.use_rga_conversion) //如果不用rga硬件转换，就没必要继续下去了
    {
        return;
    }
                               ...
                               RgaYuvConverterContext &context)
{
    ...
    //如果这个context配置rga内核失败了，就标记熔断，以后用cpu
    ...
    const int src_stride = width * src_bpp; // 官方示例中使用实际像素宽度*字节数 每行字节数
    const int dst_stride = width * dst_bpp; // 官方示例中使用实际像素宽度*字节数 每行字节数
    const size_t src_bytes = static_cast<size_t>(src_stride) * static_cast<size_t>(height); //总字节数
    const size_t dst_bytes = static_cast<size_t>(dst_stride) * static_cast<size_t>(height); //总字节数
    ...
    bool src_ok = runtime_cache.src_buffer.ensure(aligned_src_bytes); //ensure函数，用来mmap并且申请到一个handle
    bool dst_ok = runtime_cache.dst_buffer.ensure(aligned_dst_bytes);

    if (src_ok && dst_ok)
    {
        // 复制源数据到缓冲区
        unsigned char *src_ptr = reinterpret_cast<unsigned char *>(runtime_cache.src_buffer.addr()); //RGA对应的buf
        unsigned char *src_original = reinterpret_cast<unsigned char *>(src_buffer);//用户自己定义的buf，这里的目的都是让指针变成可以按字节进行访问
